...
const TABS: { href: string; label: string; icon: LucideIcon }[] = [
  ...
  { href: "/prof/mensagens", label: "Mensagens", icon: MessageSquare },
    ...
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-hairline bg-panel/90 pb-[env(safe-area-inset-bottom)] backdrop-blur-sm">
      <ul className="mx-auto grid max-w-md grid-cols-5">
        {TABS.map(({ href, label, icon: Icon }) => {
          ...
          const showDot = href === "/prof/mensagens" && unreadMessages > 0;
          return (
            <li key={href}>
              <Link
                href={href}
              ...
              >
                <span className="relative">
                  ...
                  {showDot ? (
                    <span
                      aria-hidden
                      className="absolute -right-1 -top-0.5 size-2 rounded-full bg-brand ring-2 ring-panel"
                    />
                  ) : null}
                </span>
                {label}
